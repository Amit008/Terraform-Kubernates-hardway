...
resource "null_resource" "Generate-Kubelet-Client-Certificates" {
 ...
 depends_on       = ["null_resource.Generate-CA"]
  provisioner "local-exec" {
  command = "cfssl gencert -ca=../../Certificate/ca.pem -ca-key=../../Certificate/ca-key.pem -config=../../JasonFiles/ca-config.json -hostname=worker-${count.index},element("${var.external_ip}","${count.index}") -profile=kubernetes ../../JasonFiles/worker-${count.index}-csr.json | cfssljson -bare ../../Certificate/worker-${count.index}"
 }
}
...
resource "null_resource" "API-Server-Certificate" {
 ...
 depends_on       = ["null_resource.Generate-CA"]
  provisioner "local-exec" {
  command = "cfssl gencert -ca=../../Certificate/ca.pem -ca-key=../../Certificate/ca-key.pem -config=../../JasonFiles/ca-config.json -hostname=10.32.0.1,10.240.0.10,10.240.0.11,10.240.0.12,${var.KUBERNETES_PUBLIC_ADDRESS},127.0.0.1,${var.KUBERNETES_HOSTNAMES} -profile=kubernetes ../../JasonFiles/kubernetes-csr.json | cfssljson -bare ../../Certificate/kubernetes"
}
